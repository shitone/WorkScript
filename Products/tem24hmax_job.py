...
def _job(now):
    ...
    title1 = u'江西省日最高气温' + (bjnow-datetime.timedelta(hours=24)).strftime(u'%m月%d日')
    title2 = bjnow.strftime(u'%Y年%m月%d日%H时')+u'制作'
    fn = "SURF_TEM_24HMAX_" + timestr + ".png"
    # x, y, z = cimissdata.get_jx_pre_1h(timestr)
    x, y, z = cimissdata.get_jx_multi_h(24, 'TEM_Max', timestr)
    ...
    if pftp.connect():
       if pftp.upload(upload_path='SURF/TEM/24HMAX/', local_path='source', upload_file=fn):
          is_success = True
       pftp.dis_connect()
    if is_success:
