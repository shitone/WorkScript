...
def _job(now):
    ...
    if now.hour == 0 or now.hour == 12:
        title1 = (bjnow - datetime.timedelta(hours=24)).strftime(u'江西省%H时') + bjnow.strftime(u'-%H时降水') + (bjnow - datetime.timedelta(hours=24)).strftime(u'%m月%d日-') + bjnow.strftime(u'%m月%d日')
        title2 = bjnow.strftime(u'%Y年%m月%d日%H时制作')
