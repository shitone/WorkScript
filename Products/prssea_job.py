...
def _job(now):
    ...
    title1 = u'江西省海平面气压' + bjnow.strftime(u'%m月%d日') + (bjnow-datetime.timedelta(hours=1)).strftime(u'%H时-') + bjnow.strftime(u'%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "SURF_PRS_SEA_" + timestr + ".png"
    x, y, z, _ = cimissdata.get_jx_1h('PRS_Sea', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/PRS/SEA/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
