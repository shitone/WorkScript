...
def _job(now):
    ...
    title1 = u'江西省最小相对湿度' + bjnow.strftime(u'%m月%d日') + bjnow.strftime(u'%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "SURF_RHU_MIN_" + timestr + ".png"
    x, y, z, _ = cimissdata.get_jx_1h('RHU_Min', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/RHU/MIN/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
