...
def _job(now):
    ...
    title1 = u'江西省近3小时降水' + (bjnow-datetime.timedelta(hours=3)).strftime(u'%m月%d日%H时-') + bjnow.strftime(u'%m月%d日%H时')
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/PRE/3H/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
