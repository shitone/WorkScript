...
def _job(now):
    ...
    title1 = u'江西省逐12小时降水' + bjnow.strftime(u'%m月%d日') + (bjnow-datetime.timedelta(hours=12)).strftime(u'%H时-') + bjnow.strftime(u'%H时')
    title2 = bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "SURF_PRE_12H_" + timestr + ".png"
    x, y, z = cimissdata.get_jx_multi_h(12, 'PRE_1h', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    drawmap = DrawMap(levels=list(eval(config.get('Draw', 'PRE_12H_LEVELS'))),
                    colors=list(eval(config.get('Draw', 'PRE_12H_COLORS'))),
                    cheight="25%",
                    unit=config.get('Draw', 'PRE_12H_UNIT'),
    ...
    if pftp.connect():
        for fn in os.listdir('source'):
            ...
            if pftp.upload(upload_path='SURF/PRE/12H/', local_path='source', upload_file=fn):
                is_success = True
        pftp.dis_connect()
    if is_success:
    ...
    if now.hour == 0 or now.hour == 12:
        title1 = (bjnow - datetime.timedelta(hours=12)).strftime(u'江西省%H时') + bjnow.strftime(u'-%H时降水%m月%d日') + bjnow.strftime(u'%m月%d日')
        title2 = bjnow.strftime(u'%Y年%m月%d日%H时制作')
