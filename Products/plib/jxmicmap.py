...
class DrawMap(object):
    ...
    def draw_vector_map(self, x, y, u, v):
        config = ConfigParser.RawConfigParser(allow_no_value=True)
        config.read('config.txt')
        fig = plt.figure(figsize=(7,8), frameon=False, edgecolor='none')
        ax = fig.add_subplot(1, 1, 1, projection=ccrs.Mercator())
        ...
        # cs_angle.set_clip_path(patch)
        # cs_angle.arrows.set_clip_path(patch)
        # cs_angle.lines.set_clip_path(patch)
        for artist in ax.get_children():
            from matplotlib.patches import FancyArrowPatch
            from matplotlib.collections import LineCollection
            from matplotlib.quiver import Barbs
            if isinstance(artist, FancyArrowPatch) or isinstance(artist, LineCollection) or isinstance(artist, Barbs):
