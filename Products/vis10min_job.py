...
def _job(now):
    ...
    title1 = u'江西省水平能见度' + bjnow.strftime(u'%m月%d日%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "SURF_VIS_10MIN_" + timestr + ".png"
    x, y, z, _ = cimissdata.get_jx_1h('VIS_HOR_10MI', timestr)
    maxpre = round(max(z) / 1000.0, 1)
    minpre = round(min(z) / 1000.0, 1)
    ...
    drawmap = DrawMap(levels=list(eval(config.get('Draw', 'VIS_LEVELS'))),
                    colors=list(eval(config.get('Draw', 'VIS_COLORS'))),
                    cheight="45%",
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/VIS/10MIN/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
