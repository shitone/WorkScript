...
def _job(now):
    ...
    title1 = u'江西省最高地面温度' + bjnow.strftime(u'%m月%d日%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "SURF_GST_MAX_" + timestr + ".png"
    x, y, z, _ = cimissdata.get_jx_1h('GST_Max', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/GST/MAX/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
