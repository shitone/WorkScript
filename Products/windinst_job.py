...
def _job(now):
    ...
    title1 = u'江西省瞬时风速风向' + bjnow.strftime(u'%m月%d日') + (bjnow-datetime.timedelta(hours=1)).strftime(u'%H时-') + bjnow.strftime(u'%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    x, y, z, angle = cimissdata.get_jx_1h('WIN_Inst', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    fn = "SURF_WIND_INST_" + timestr + "_STREAM.png"
    ...
    if pftp.connect():
        if pftp.upload(upload_path='SURF/WIND/INST/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
    ...
    fn = "SURF_WIND_INST_" + timestr + "_BARB.png"
