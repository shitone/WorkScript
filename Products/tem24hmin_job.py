...
def _job(now):
    if now.hour == 12:
        ...
        title1 = u'江西省日最低气温' + bjnow.strftime(u'%m月%d日') + bjnow.strftime(u'%H时')
        title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
        fn = "SURF_TEM_24HMIN_" + timestr + ".png"
        x, y, z = cimissdata.get_jx_multi_h(4, 'TEM_Min', timestr, 6)
        ...
        if pftp.connect():
            if pftp.upload(upload_path='SURF/TEM/24HMIN/', local_path=source_path, upload_file=fn):
                is_success = True
            pftp.dis_connect()
        if is_success:
