...
def _job(now):
    ...
    timestr = (now-datetime.timedelta(hours=2)).strftime("%Y%m%d%H0000")
    title1 = u'江西省大气可降水量' + (bjnow-datetime.timedelta(hours=2)).strftime(u'%m月%d日%H时')
    title2 = u'' + bjnow.strftime(u'%Y年%m月%d日%H时制作')
    fn = "UPAR_PREPREFORE_1H_" + timestr + ".png"
    x, y, z, _ = cimissdata.get_jx_1h('PRE_PRE_Fore', timestr)
    maxpre = max(z)
    minpre = min(z)
    ...
    drawmap = DrawMap(levels=list(eval(config.get('Draw', 'PRE_PRE_FORE_LEVELS'))),
                    colors=list(eval(config.get('Draw', 'PRE_PRE_FORE_COLORS'))),
                    cheight="50%",
                    unit=config.get('Draw', 'PRE_PRE_FORE_UNIT'),
    ...
    if pftp.connect():
        if pftp.upload(upload_path='UPAR/PREPREFORE/1H/', local_path=source_path, upload_file=fn):
            is_success = True
        pftp.dis_connect()
    if is_success:
